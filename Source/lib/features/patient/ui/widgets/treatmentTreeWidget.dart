...
class TreatmentTreeWidget extends StatefulWidget {
  TreatmentTreeWidget();

  @override
  _TreatmentTreeWidgetWidgetState createState() =>
      _TreatmentTreeWidgetWidgetState();
}

class _TreatmentTreeWidgetWidgetState extends State<TreatmentTreeWidget> {
  ...
  List<Node<Treatment>> convertToNodeList(
      List<Treatment> totalDiagnosis, Node? node) {
    return totalDiagnosis
        .map((Treatment e) {
          List<Node<Treatment>> _children = [];
          if (e.children != null) {
          ...
          int pos = patientBloc!.state.selectedTreatment.indexWhere((element) {
            return e.id.toString() == element['key'];
          });

          if (pos >= 0) isSelected = true;

          return Node<Treatment>(
            key: tmpKey,
  ...
  @override
  void initState() {
    var totalTreatment = context.read<PatientBloc>().state.treatments;
    patientBloc = context.read<PatientBloc>();
    _nodes = convertToNodeList(totalTreatment, null);
      ...
      Container(
        ...
        child: TreeView(
          ...
          onNodeTap: (key) {
            ...
            context.read<PatientBloc>().add(PatientDialogTreatmentSelected(
                node!.key.split('_')[0], node.label));

            var totalTreatment = context.read<PatientBloc>().state.treatments;

            setState(() {
              _nodes = convertToNodeList(totalTreatment, node);
              _treeViewController =
