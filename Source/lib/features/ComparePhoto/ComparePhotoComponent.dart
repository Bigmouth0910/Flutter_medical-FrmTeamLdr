...
import 'package:rxphoto/features/ComparePhoto/ui/injectionInputDialog.dart';
...
class ComparePhotoComponent extends StatefulWidget {
  PatientImage photo;
  String positionType;
  ComparePhotoComponent(this.photo, this.positionType);
  @override
  _ComparePhotoComponentState createState() => _ComparePhotoComponentState();
  static _ComparePhotoComponentState of(BuildContext context) =>
      context.findAncestorStateOfType<_ComparePhotoComponentState>()!;
}

class _ComparePhotoComponentState extends State<ComparePhotoComponent> {
  ...
  void getDimensionsOfFile() async {
    ...
    final selectedGalleryPhoto = widget.photo;
...
//Safe area in logical pixels
      ...
      setState(() {
        calculatedCanvasHeight =
            photoHeight * 1.0 * (safeWidth / 2 / photoWidth);
      });
    }));
  }
  ...
  Widget build(BuildContext context) {
    ...
    final selectedPhotoUrl = "$patientImgUrl/${widget.photo.photoUrl}";
    ...
    return Scaffold(
        backgroundColor: widget.positionType == 'left'
            ? Color(0xff89F8FF)
            : Color(0xff9D7BFD),
        floatingActionButton: windowState == WindowType.edit
            ? FloatingActionButton(
                heroTag: widget.positionType,
  ...
  void onHandlerToSave() async {
    ...
    var sendData = {
      ...
      "medical_history_id": selectedGalleryPhoto.medicalHistoryId
    };
            ...
            drawPoints.entries.forEach((element) {
              for (CPoint value in element.value) {
                if (value.isSelected == true) {
                  final deltaPoint = details.delta;
